...
const SECTION2_MODEL_SCROLL_SPIN_FACTOR = 0.4;
...
async function initSection2Model() {
  ...
  try {
    ...
    section2ModelMount.appendChild(renderer.domElement);
    ...
    modelTiltGroup.rotation.z = THREE.MathUtils.degToRad(35);
    modelSpinGroup.add(root);
    if (section2ModelScene) {
      section2ModelScene.modelRoot = root;
      section2ModelScene.baseScale = baseScale;
      section2ModelScene.modelSpinGroup = modelSpinGroup;
      section2ModelScene.modelTiltGroup = modelTiltGroup;
    }

    const resize = () => {
      const width = Math.max(1, section2ModelMount.clientWidth);
      const height = Math.max(1, section2ModelMount.clientHeight);
    ...
    const animationLoop = () => {
      ...
      if (modelSpinGroup && !section2ModelScene?.isHovering) {
        const now = performance.now();
      ...
      if (pointerDirty && pointerInside && modelRoot) {
        const rect = renderer.domElement.getBoundingClientRect();
        pointerNdc.x = ((pointerClientX - rect.left) / rect.width) * 2 - 1;
        pointerNdc.y = -((pointerClientY - rect.top) / rect.height) * 2 + 1;
        raycaster.setFromCamera(pointerNdc, camera);
        const intersects = raycaster.intersectObject(modelRoot, true);
        animateModelHover(intersects.length > 0);
        pointerDirty = false;
      }
    ...
    const animateModelHover = (isHovering) => {
      if (!section2ModelScene) {
        return;
      }
      section2ModelScene.isHovering = isHovering;
      const activeRoot = modelRoot || section2ModelScene.modelRoot;
      const activeSpinGroup = section2ModelScene.modelSpinGroup || modelSpinGroup;
      const activeBaseScale = section2ModelScene.baseScale || baseScale;
      if (!activeRoot || !activeSpinGroup || !window.gsap) {
        return;
      }
      window.gsap.killTweensOf(activeSpinGroup.rotation);
      window.gsap.killTweensOf(activeRoot.scale);
      if (isHovering) {
        window.gsap.to(activeSpinGroup.rotation, {
          y: 0,
          duration: 0.3,
          ease: 'power2.out'
        });
      }
      const targetScale = isHovering ? activeBaseScale * 0.1 : activeBaseScale;
      window.gsap.to(activeRoot.scale, {
        x: targetScale,
        y: targetScale,
        z: targetScale,
        duration: 0.3,
    ...
    const updateModelHoverFromPointer = (event) => {
      if (!section2ModelScene?.isActive || !modelRoot) {
        return;
      }
      pointerInside = true;
    ...
    renderer.domElement.addEventListener('pointermove', updateModelHoverFromPointer);
    renderer.domElement.addEventListener('pointerleave', () => {
      pointerInside = false;
      pointerDirty = false;
      animateModelHover(false);
    });
  } catch (error) {
    console.error('Section 2 model failed to load.', error);
    section2ModelMount.innerHTML = '';
  }
}
