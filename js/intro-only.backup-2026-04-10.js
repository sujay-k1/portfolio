...
const section1FocusAnchor = document.getElementById('section1-focus-anchor');
const section2FocusAnchor = document.getElementById('section2-focus-anchor');
...
const SNAP_STATE = {
  ...
  pendingSectionFromIndex: null,
  pendingSectionAllowIncomingCarry: false,
  touchStartY: 0,
  touchLastY: 0,
  touchLastTs: 0,
...
const FINAL_HORIZON_SPECIAL_INDEX = 0;
const FINAL_HORIZON_DEFAULT_INDEX = 1;
const FINAL_HORIZON_FOCUSABLE_SELECTOR = [
  'a[href]',
  'button:not([disabled])',
  'input:not([disabled])',
  'select:not([disabled])',
...
const FINAL_HORIZON_STATE = {
  ...
  expanded: false,
  hasEntered: false,
  enteredShellIndex: null,
  pendingKeyboardEntryIndex: null,
  pendingKeyboardEntryFocus: false,
...
function renderFinalHorizonSection() {
  ...
  finalHorizonRail.innerHTML = finalHorizonCardsData.map((card, index) => {
    ...
    return `
      <article
        ...
        data-card-index="${index}"
        ${card.href ? `data-href="${card.href}" ${card.newTab ? 'data-new-tab="true"' : ''} role="link"` : ''}
  ...
  syncFinalHorizonCardShellTabStops();
}

function getFinalHorizonCardShell(target) {
  if (!(target instanceof Element)) {
    return null;
  }
  const shell = target.closest('.folio-hcard-wrap');
  return shell && FINAL_HORIZON_STATE.cards.includes(shell) ? shell : null;
}

function getFinalHorizonCardShellIndex(target) {
  const shell = getFinalHorizonCardShell(target);
  if (!shell) {
    return -1;
  }
  return Number(shell.dataset.cardIndex || -1);
}

function getFinalHorizonDefaultShellIndex() {
  if (!FINAL_HORIZON_STATE.cards.length) {
    return 0;
  }
  return Math.min(FINAL_HORIZON_DEFAULT_INDEX, FINAL_HORIZON_STATE.cards.length - 1);
}

function getFinalHorizonLastShellIndex() {
  return Math.max(FINAL_HORIZON_STATE.cards.length - 1, 0);
}

function getFinalHorizonActiveCardShell({ forTabOrder = false } = {}) {
  if (!FINAL_HORIZON_STATE.cards.length) {
    return null;
  }
  const activeIndex = forTabOrder && SNAP_STATE.index !== finalHorizonSectionIndex
    ? getFinalHorizonDefaultShellIndex()
    : clamp(FINAL_HORIZON_STATE.snapIndex, 0, FINAL_HORIZON_STATE.cards.length - 1);
  return FINAL_HORIZON_STATE.cards[activeIndex] || null;
}

function cacheFinalHorizonManagedTabIndex(node) {
  if (!(node instanceof HTMLElement) || node.dataset.finalHorizonTabManaged === 'true') {
    return;
  }
  node.dataset.finalHorizonTabManaged = 'true';
  node.dataset.finalHorizonHadTabindex = node.hasAttribute('tabindex') ? 'true' : 'false';
  if (node.hasAttribute('tabindex')) {
    node.dataset.finalHorizonOriginalTabindex = node.getAttribute('tabindex') || '';
  }
}

function setFinalHorizonFocusableEnabled(node, enabled) {
  if (!(node instanceof HTMLElement)) {
    return;
  }
  cacheFinalHorizonManagedTabIndex(node);
  if (enabled) {
    if (node.dataset.finalHorizonHadTabindex === 'true') {
      node.setAttribute('tabindex', node.dataset.finalHorizonOriginalTabindex || '0');
    } else {
...
function getFinalHorizonInnerFocusables(card) {
  if (!(card instanceof Element)) {
    return [];
  }
  return Array.from(card.querySelectorAll(FINAL_HORIZON_FOCUSABLE_SELECTOR)).filter((node) => node !== card);
}

function syncFinalHorizonInnerControlTabStops() {
  FINAL_HORIZON_STATE.cards.forEach((card, index) => {
    const enabled =
      SNAP_STATE.index === finalHorizonSectionIndex &&
      FINAL_HORIZON_STATE.enteredShellIndex === index;
    getFinalHorizonInnerFocusables(card).forEach((node) => {
      setFinalHorizonFocusableEnabled(node, enabled);
    });
  });
}

function resetFinalHorizonEnteredShell() {
  if (FINAL_HORIZON_STATE.enteredShellIndex == null) {
    return;
  }
  FINAL_HORIZON_STATE.enteredShellIndex = null;
  syncFinalHorizonInnerControlTabStops();
}

function focusElementWithoutScroll(node) {
  if (!(node instanceof HTMLElement)) {
    return;
  }
  try {
...
function focusFinalHorizonCardShell(card = getFinalHorizonActiveCardShell()) {
  if (!card) {
    return;
  }
  focusElementWithoutScroll(card);
}

function syncFinalHorizonCardShellTabStops({ activeIndex = null, focusTarget = null } = {}) {
  const resolvedIndex = activeIndex == null
    ? (SNAP_STATE.index === finalHorizonSectionIndex
        ? clamp(FINAL_HORIZON_STATE.snapIndex, 0, FINAL_HORIZON_STATE.cards.length - 1)
        : getFinalHorizonDefaultShellIndex())
    : clamp(activeIndex, 0, FINAL_HORIZON_STATE.cards.length - 1);
  FINAL_HORIZON_STATE.cards.forEach((card) => {
    card.tabIndex = Number(card.dataset.cardIndex || -1) === resolvedIndex ? 0 : -1;
  });
  syncFinalHorizonInnerControlTabStops();
  if (focusTarget != null) {
    focusFinalHorizonCardShell(FINAL_HORIZON_STATE.cards[clamp(focusTarget, 0, FINAL_HORIZON_STATE.cards.length - 1)]);
  }
}

function enterFinalHorizonShellControls(shell) {
  const shellIndex = getFinalHorizonCardShellIndex(shell);
  if (shellIndex < 0) {
    return false;
  }
  const focusables = getFinalHorizonInnerFocusables(shell);
  if (!focusables.length) {
    return false;
  }
  FINAL_HORIZON_STATE.enteredShellIndex = shellIndex;
  syncFinalHorizonInnerControlTabStops();
  focusElementWithoutScroll(focusables[0]);
  return true;
}

function isFinalHorizonShellFocused() {
  return !!document.activeElement && document.activeElement instanceof Element && FINAL_HORIZON_STATE.cards.includes(document.activeElement);
}

function getFinalHorizonCollapsedTargetForCardOne() {
  const cardOne = FINAL_HORIZON_STATE.cards[FINAL_HORIZON_DEFAULT_INDEX];
  if (!cardOne) {
    return FINAL_HORIZON_STATE.snapPoints[FINAL_HORIZON_DEFAULT_INDEX] ?? 0;
  }
  return (
    ...
    ? (FINAL_HORIZON_STATE.snapPoints[FINAL_HORIZON_DEFAULT_INDEX] ?? 0)
    : Math.max(
        0,
...
function setFinalOpportunityExpandedImmediate(expanded) {
  const card = FINAL_HORIZON_STATE.cards[FINAL_HORIZON_SPECIAL_INDEX];
  if (!card) {
    return;
  }
  if (usesFinalHorizonVerticalLayout()) {
    ...
    card.classList.remove('is-expanded', 'is-resetting');
    refreshFinalHorizonSnapPoints();
    return;
  }
...
function finishFinalHorizonEntryTween() {
  if (!FINAL_HORIZON_STATE.cards.length) {
    return;
  }
  ...
  FINAL_HORIZON_STATE.cards[FINAL_HORIZON_SPECIAL_INDEX]?.classList.remove('is-resetting');
  updateFinalHorizonVisuals();
}

function finishFinalHorizonMotion() {
  if (!FINAL_HORIZON_STATE.isAnimating) {
    return;
  }
  gsap.killTweensOf(FINAL_HORIZON_STATE);
  FINAL_HORIZON_STATE.isAnimating = false;
  FINAL_HORIZON_STATE.visualIndex = clamp(FINAL_HORIZON_STATE.snapIndex, 0, FINAL_HORIZON_STATE.cards.length - 1);
  const targetX = FINAL_HORIZON_STATE.pendingTargetX
    ?? FINAL_HORIZON_STATE.snapPoints[FINAL_HORIZON_STATE.snapIndex]
    ?? FINAL_HORIZON_STATE.x;
  FINAL_HORIZON_STATE.pendingTargetX = null;
  setFinalHorizonScrollPosition(targetX);
  recordMainScrollDebugMotion('end');
  settleFinalHorizonCardSnap();
}

function settleFinalHorizonForKeyboardInput() {
  cancelFinalHorizonTouchMomentum();
  finishFinalHorizonEntryTween();
  finishFinalHorizonMotion();
  resetFinalHorizonEnteredShell();
}

function requestFinalHorizonKeyboardEntry(targetIndex, { focusShell = false } = {}) {
  FINAL_HORIZON_STATE.pendingKeyboardEntryIndex = clamp(
    targetIndex,
    0,
    Math.max(FINAL_HORIZON_STATE.cards.length - 1, 0)
  );
  FINAL_HORIZON_STATE.pendingKeyboardEntryFocus = focusShell;
  syncFinalHorizonCardShellTabStops({ activeIndex: FINAL_HORIZON_STATE.pendingKeyboardEntryIndex });
}

function consumeFinalHorizonKeyboardEntry() {
  const index = FINAL_HORIZON_STATE.pendingKeyboardEntryIndex;
  const shouldFocus = FINAL_HORIZON_STATE.pendingKeyboardEntryFocus;
  FINAL_HORIZON_STATE.pendingKeyboardEntryIndex = null;
  FINAL_HORIZON_STATE.pendingKeyboardEntryFocus = false;
  return {
    index: index == null ? null : clamp(index, 0, Math.max(FINAL_HORIZON_STATE.cards.length - 1, 0)),
    shouldFocus
  };
}

function cancelAnimatedSectionTransitionForKeyboard() {
  if (!SNAP_STATE.isTransitioning && !animatedTransition) {
    return;
  }
  const stage = ensureAnimatedStage();
...
function interruptMainSectionMotionForKeyboard() {
  if (SNAP_STATE.isAnimating) {
    gsap.killTweensOf(snapTrack);
    SNAP_STATE.isAnimating = false;
    recordMainScrollDebugMotion('end');
  }
  cancelAnimatedSectionTransitionForKeyboard();
}

function finishMainSectionMotion() {
  if (!SNAP_STATE.isAnimating && !SNAP_STATE.isTransitioning) {
    return false;
  }
  const targetIndex = clamp(
    SNAP_STATE.pendingSectionIndex ?? SNAP_STATE.index,
    0,
    snapSections.length - 1
  );
  const previousIndex = clamp(
    SNAP_STATE.pendingSectionFromIndex ?? SNAP_STATE.index,
    0,
    snapSections.length - 1
  );
  const allowIncomingCarry = SNAP_STATE.pendingSectionAllowIncomingCarry;
  const targetY = -targetIndex * getSnapViewportHeightPx();
  gsap.killTweensOf(snapTrack);
  SNAP_STATE.isAnimating = false;
  cancelAnimatedSectionTransitionForKeyboard();
  recordMainScrollDebugMotion('end');
  gsap.set(snapTrack, { y: targetY });
  SNAP_STATE.index = targetIndex;
  SNAP_STATE.pendingSectionIndex = null;
  SNAP_STATE.pendingSectionFromIndex = null;
  SNAP_STATE.pendingSectionAllowIncomingCarry = false;
  syncBodySectionState(targetIndex);
  updateSectionOneWaveVisibilityFromTrackPosition(targetY);
  refreshSection2ModelVisibility();
  if (targetIndex === finalHorizonSectionIndex) {
    activateFinalHorizonSection(previousIndex, { settleEntryTween: true });
  } else {
    hideFinalHorizonLottieMarker();
  }
  activateSectionAnimation(targetIndex);
  settleAfterSectionChange();
  if (allowIncomingCarry && isSection2FlowIndex(targetIndex)) {
    SNAP_STATE.wheelCooldownUntil = 0;
    if (isSection2TextIndex(targetIndex)) {
...
function isFinalHorizonEntryTweenActive() {
  return !!FINAL_HORIZON_STATE.cards.length && gsap.isTweening(FINAL_HORIZON_STATE.cards);
}

function settleForKeyboardNavigation() {
  if (SNAP_STATE.isAnimating || SNAP_STATE.isTransitioning) {
    finishMainSectionMotion();
  } else if (FINAL_HORIZON_STATE.isAnimating || isFinalHorizonEntryTweenActive()) {
    settleFinalHorizonForKeyboardInput();
  }
}

function moveFinalHorizonShellFocusImmediate(targetIndex, { focusShell = true } = {}) {
  if (!FINAL_HORIZON_STATE.cards.length) {
    return false;
  }
  settleFinalHorizonForKeyboardInput();
  const clamped = clamp(targetIndex, 0, FINAL_HORIZON_STATE.cards.length - 1);
  if (clamped === FINAL_HORIZON_SPECIAL_INDEX) {
    setFinalOpportunityExpandedImmediate(true);
  } else {
    setFinalOpportunityExpandedImmediate(false);
  }
  FINAL_HORIZON_STATE.snapIndex = clamped;
  const targetX =
    clamped === FINAL_HORIZON_DEFAULT_INDEX && FINAL_HORIZON_STATE.expanded
      ? getFinalHorizonCollapsedTargetForCardOne()
      : (FINAL_HORIZON_STATE.snapPoints[clamped] ?? 0);
  if (clamped !== FINAL_HORIZON_SPECIAL_INDEX) {
    MAIN_SCROLL_DEBUG_STATE.requireFreshSpecialExit = false;
    updateMainScrollDebugHud();
  }
  goToFinalHorizonCard(clamped, true, targetX);
  FINAL_HORIZON_STATE.pendingKeyboardEntryIndex = null;
  FINAL_HORIZON_STATE.pendingKeyboardEntryFocus = false;
  syncFinalHorizonCardShellTabStops({
    activeIndex: clamped,
    focusTarget: focusShell ? clamped : null
  });
  return true;
}

function moveFinalHorizonShellFocus(targetIndex, { focusShell = true, immediate = false } = {}) {
  if (!FINAL_HORIZON_STATE.cards.length) {
    return false;
  }
  if (immediate) {
    return moveFinalHorizonShellFocusImmediate(targetIndex, { focusShell });
  }
  resetFinalHorizonEnteredShell();
  const clamped = clamp(targetIndex, 0, FINAL_HORIZON_STATE.cards.length - 1);
  let targetX = FINAL_HORIZON_STATE.snapPoints[clamped] ?? 0;
  if (clamped === FINAL_HORIZON_SPECIAL_INDEX) {
    ...
    MAIN_SCROLL_DEBUG_STATE.requireFreshSpecialExit = false;
    updateMainScrollDebugHud();
    if (FINAL_HORIZON_STATE.snapIndex === FINAL_HORIZON_SPECIAL_INDEX && FINAL_HORIZON_STATE.expanded) {
      setFinalOpportunityExpanded(false);
      if (clamped === FINAL_HORIZON_DEFAULT_INDEX) {
        targetX = getFinalHorizonCollapsedTargetForCardOne();
      }
    }
  }
  FINAL_HORIZON_STATE.pendingKeyboardEntryIndex = null;
  FINAL_HORIZON_STATE.pendingKeyboardEntryFocus = false;
  goToFinalHorizonCard(clamped, false, targetX);
  if (focusShell && !isFinalHorizonShellFocused()) {
    requestAnimationFrame(() => {
      focusFinalHorizonCardShell(FINAL_HORIZON_STATE.cards[clamped]);
    });
  }
  return true;
}

function isFinalHorizonDescendantInteractionTarget(target) {
  if (!(target instanceof Element)) {
    return false;
  }
  const shell = getFinalHorizonCardShell(target);
  return !!shell && target !== shell;
}

function focusSectionAnchorForKeyboard(anchor, sectionIndex) {
  ...
  FINAL_HORIZON_STATE.pendingKeyboardEntryFocus = false;
  resetFinalHorizonEnteredShell();
  settleForKeyboardNavigation();
  if (sectionIndex !== finalHorizonSectionIndex && SNAP_STATE.index !== sectionIndex) {
    goToSection(sectionIndex);
  }
  syncFinalHorizonCardShellTabStops();
  focusElementWithoutScroll(anchor);
}

function focusBrandFromFinalHorizonLoop() {
  FINAL_HORIZON_STATE.pendingKeyboardEntryIndex = null;
  FINAL_HORIZON_STATE.pendingKeyboardEntryFocus = false;
  resetFinalHorizonEnteredShell();
  settleForKeyboardNavigation();
  focusElementWithoutScroll(brandLink);
  if (SNAP_STATE.index !== 0) {
    goToSection(0);
  }
  syncFinalHorizonCardShellTabStops();
}

function focusFinalHorizonShellForKeyboard(targetIndex) {
  resetFinalHorizonEnteredShell();
  settleForKeyboardNavigation();
  if (SNAP_STATE.index !== finalHorizonSectionIndex) {
    requestFinalHorizonKeyboardEntry(targetIndex, { focusShell: true });
    goToSection(finalHorizonSectionIndex);
    return;
  }
  if (clamp(targetIndex, 0, FINAL_HORIZON_STATE.cards.length - 1) === FINAL_HORIZON_STATE.snapIndex) {
    syncFinalHorizonCardShellTabStops({
      activeIndex: FINAL_HORIZON_STATE.snapIndex,
      focusTarget: targetIndex
    });
    return;
  }
  moveFinalHorizonShellFocus(targetIndex, { focusShell: true });
}

function handleFinalHorizonKeyboardMove(direction) {
  if (!direction || !FINAL_HORIZON_STATE.cards.length) {
    return false;
  }
  settleForKeyboardNavigation();
  if (SNAP_STATE.index !== finalHorizonSectionIndex) {
    requestFinalHorizonKeyboardEntry(getFinalHorizonDefaultShellIndex(), { focusShell: true });
    goToSection(finalHorizonSectionIndex);
    return true;
  }
  const lastShellIndex = getFinalHorizonLastShellIndex();
  if (direction < 0) {
    if (FINAL_HORIZON_STATE.snapIndex > FINAL_HORIZON_DEFAULT_INDEX) {
      return moveFinalHorizonShellFocus(FINAL_HORIZON_STATE.snapIndex - 1, { focusShell: true });
    }
    if (FINAL_HORIZON_STATE.snapIndex === FINAL_HORIZON_DEFAULT_INDEX) {
      return moveFinalHorizonShellFocus(FINAL_HORIZON_SPECIAL_INDEX, { focusShell: true });
    }
    goToSection(finalHorizonSectionIndex - 1, false, { allowIncomingCarry: true });
    return true;
  }
  if (FINAL_HORIZON_STATE.snapIndex === FINAL_HORIZON_SPECIAL_INDEX) {
    return moveFinalHorizonShellFocus(FINAL_HORIZON_DEFAULT_INDEX, { focusShell: true });
  }
  if (FINAL_HORIZON_STATE.snapIndex < lastShellIndex) {
    return moveFinalHorizonShellFocus(FINAL_HORIZON_STATE.snapIndex + 1, { focusShell: true });
  }
  settleFinalHorizonCardSnap();
  return true;
...
function goToFinalHorizonCard(index, immediate = false, targetOverride = null) {
  ...
  const shouldRefocusShell = isFinalHorizonShellFocused();
  FINAL_HORIZON_STATE.snapIndex = clamped;
  const targetX = targetOverride ?? FINAL_HORIZON_STATE.snapPoints[clamped] ?? 0;
  FINAL_HORIZON_STATE.pendingTargetX = targetX;
  syncFinalHorizonCardShellTabStops({
    activeIndex: clamped,
    focusTarget: shouldRefocusShell ? clamped : null
  });
...
function activateFinalHorizonSection(fromIndex, options = {}) {
  const { settleEntryTween = false } = options;
  if (!FINAL_HORIZON_STATE.cards.length) {
    return;
  }
  const keyboardEntry = fromIndex !== finalHorizonSectionIndex
    ? consumeFinalHorizonKeyboardEntry()
    : { index: null, shouldFocus: false };
  const requestedIndex = keyboardEntry.index ?? getFinalHorizonDefaultShellIndex();
  const applyRequestedShell = () => {
    goToFinalHorizonCard(requestedIndex, true);
    syncFinalHorizonCardShellTabStops({ activeIndex: requestedIndex });
    if (keyboardEntry.shouldFocus) {
      requestAnimationFrame(() => {
        focusFinalHorizonCardShell(FINAL_HORIZON_STATE.cards[requestedIndex]);
      });
    }
  };
  ...
  if (fromIndex !== finalHorizonSectionIndex) {
    const runActivation = () => {
      ...
      if (isSection2FlowIndex(fromIndex)) {
        MAIN_SCROLL_DEBUG_STATE.requireFreshSection3Entry = keyboardEntry.shouldFocus
          ? false
          : !usesFinalHorizonVerticalLayout();
        SNAP_STATE.wheelCooldownUntil = 0;
        applyRequestedShell();
      } else {
        applyRequestedShell();
    ...
    if (settleEntryTween) {
      ...
      requestAnimationFrame(runActivation);
...
function handleSection2Scroll(deltaY, options = {}) {
  const { focusFinalHorizonOnExit = false } = options;
  ...
  if (Math.abs(SNAP_STATE.section2EdgeAccumulator) >= WHEEL_SNAP_THRESHOLD) {
    ...
    if (focusFinalHorizonOnExit && edgeDirection > 0) {
      requestFinalHorizonKeyboardEntry(getFinalHorizonDefaultShellIndex(), { focusShell: true });
    }
...
function goToSection(nextIndex, immediate = false, options = {}) {
  ...
  SNAP_STATE.isAnimating = true;
  SNAP_STATE.pendingSectionIndex = clamped;
  SNAP_STATE.pendingSectionFromIndex = previousIndex;
  SNAP_STATE.pendingSectionAllowIncomingCarry = allowIncomingCarry;
  recordMainScrollDebugMotion('start');
...
function initSnapScroll() {
  ...
  window.addEventListener(
    'keydown',
    (event) => {
      const isFinalHorizonInnerControl = isFinalHorizonDescendantInteractionTarget(event.target);
      if (SNAP_STATE.index === finalHorizonSectionIndex && !isFinalHorizonInnerControl) {
        if (
          event.key === 'ArrowDown' ||
          ...
          event.preventDefault();
          handleFinalHorizonKeyboardMove(1);
          return;
        }
        if (
          event.key === 'ArrowUp' ||
      ...
      if (isSection2FlowIndex(SNAP_STATE.index) && !SNAP_STATE.isAnimating) {
        if (event.key === 'ArrowDown' || event.key === 'PageDown' || event.key === ' ') {
          event.preventDefault();
          handleSection2Scroll(WHEEL_SNAP_THRESHOLD, { focusFinalHorizonOnExit: true });
          return;
        }
  ...
  finalHorizonSection?.addEventListener(
    'keydown',
    (event) => {
      const shell = getFinalHorizonCardShell(event.target);
      if (!shell) {
        return;
      }
      const shellIndex = getFinalHorizonCardShellIndex(shell);
      const targetIsShell = event.target === shell;
      if (event.key === 'Tab') {
        const focusables = getFinalHorizonInnerFocusables(shell);
        if (!targetIsShell) {
          ...
          if (!event.shiftKey && event.target === lastFocusable) {
            event.preventDefault();
            resetFinalHorizonEnteredShell();
            if (shellIndex === FINAL_HORIZON_SPECIAL_INDEX) {
              focusFinalHorizonShellForKeyboard(FINAL_HORIZON_DEFAULT_INDEX);
              return;
            }
            if (shellIndex === getFinalHorizonLastShellIndex()) {
              focusBrandFromFinalHorizonLoop();
              return;
            }
            focusFinalHorizonShellForKeyboard(shellIndex + 1);
            return;
          }
          return;
        }
        ...
        if (event.shiftKey) {
          event.preventDefault();
          if (shellIndex === FINAL_HORIZON_SPECIAL_INDEX) {
            focusSectionAnchorForKeyboard(section2FocusAnchor, section2TextSectionIndex);
            return;
          }
          if (shellIndex === FINAL_HORIZON_DEFAULT_INDEX) {
            focusFinalHorizonShellForKeyboard(FINAL_HORIZON_SPECIAL_INDEX);
            return;
          }
          focusFinalHorizonShellForKeyboard(shellIndex - 1);
          return;
      ...
      if (event.key === 'Enter' && targetIsShell && !shell.dataset.href) {
        if (enterFinalHorizonShellControls(shell)) {
          event.preventDefault();
        }
        return;
      }
      if (event.key === 'Escape' && !targetIsShell) {
        event.preventDefault();
        resetFinalHorizonEnteredShell();
        syncFinalHorizonCardShellTabStops({
          activeIndex: shellIndex,
  ...
  brandLink?.addEventListener(
    ...
    (event) => {
      ...
      event.preventDefault();
      focusFinalHorizonShellForKeyboard(getFinalHorizonLastShellIndex());
    },
    true
  );

  const sectionFocusStops = [
    [section1FocusAnchor, 0],
    [section2FocusAnchor, section2TextSectionIndex]
  ];

  sectionFocusStops.forEach(([anchor, sectionIndex]) => {
    anchor?.addEventListener('focus', () => {
      focusSectionAnchorForKeyboard(anchor, sectionIndex);
    });
  ...
  finalHorizonSection?.addEventListener(
    ...
    (event) => {
      ...
      if (SNAP_STATE.index !== finalHorizonSectionIndex) {
        focusFinalHorizonShellForKeyboard(shellIndex);
        return;
      }
      if (event.target === shell && shellIndex !== FINAL_HORIZON_STATE.snapIndex) {
        moveFinalHorizonShellFocus(shellIndex, { focusShell: false });
      }
    },
    true
  );
